...
public class SanphamAPIController {
    ...
    @Autowired
    private final userSearchProduct searchFacade;

    public SanphamAPIController(userSearchProduct searchFacade) {
    ...
    @Transactional
        public String searchProducts(
                ...
                @RequestParam(required = false) String tenNhaCungCap,Model model) {

            List<SearchProduct> criteria = new ArrayList<>();

            if (tenSanPham != null&& !tenSanPham.isEmpty()) {
                criteria.add(new TenSanPhamCriteria(tenSanPham));
            }

            if (tenDanhMuc != null&& !tenDanhMuc.isEmpty()) {
                criteria.add(new TenDanhMucCriteria(tenDanhMuc, sanphamRepo, danhmucRepo));
            }
            if (tenNhaCungCap != null&& !tenNhaCungCap.isEmpty()) {
                criteria.add(new NhaCungCapCriteria(tenNhaCungCap, sanphamRepo, nccRepo));
            }

            List<SanphamEntity> products = searchFacade.searchProducts(criteria);
